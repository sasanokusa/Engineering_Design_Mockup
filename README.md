...
Open WebUI 採用構成を前提にしつつ、このリポジトリでもデモ・互換確認用の簡易チャットUIを提供します。FastAPI は学校向けの業務データ処理バックエンドとして、文書基盤、比較、議事録、監査ログの責務を保持します。機微データを外部クラウドへ送る前提にはしていません。

## 今回の構成

- Backend: FastAPI
- Chat UI: FastAPI配信の簡易チャットUI。Open WebUI は外部チャットUIとして並存
...
- Chat API: 簡易チャットUI、履歴保存、一時文書添付、ツール候補表示
...
backend/
  ...
  prompts/        chat, cleanup, minutes prompts
...
`/chat/` は簡易チャットUIです。チャット履歴を保存し、`+` からセッション単位の一時文書を添付できます。Open WebUI から使う場合は `/api/openwebui/...` のツールAPIを呼びます。議事録、資料投入、文書比較の専用画面は、それぞれ `/`、`/documents/`、`/compare/` から直接確認できます。
...
簡易チャットUIは Markdown 表示とHTMLサニタイズに `marked` と `DOMPurify` を使い、`frontend/vendor/` に同梱しています。Open WebUI 連携は `/api/openwebui/...` のツールAPIとして並存します。
...
~/Documents/engineering_design_2026/data/
  ...
  audit/openwebui_bridge.jsonl
                           Open WebUI bridge ツール呼び出しの監査ログ
```

音声ファイルは通常のファイルとして `data/uploads/audio/` に置かれます。永続資料の原本は削除せず `data/originals/persistent/{collection_id}/{document_id}/source.ext` に保持します。一時文書は `data/originals/temporary/{session_idまたはexternal_session_id}/{document_id}/source.ext` に保持します。正規化済み JSON / Markdown と chunk JSONL は別管理です。チャットセッション、メッセージ、raw transcript、整形済みテキスト、議事録案、資料メタデータ、FTS5索引は `data/local_school_ai.db` の各テーブルに保存されます。チャットセッション削除時、そのセッションに紐づく一時文書は原本、正規化済みファイル、chunk JSONL、FTS行ごと削除されます。
...
5. チャット入力欄左側の `+` から一時文書を添付し、メッセージを送信する
...
9. 必要なら新規チャットで履歴保存、添付文書参照、LLM応答を確認
...
一時文書としてチャットセッションへ紐づける場合:

```bash
school-ai ingest ./docs/handbooks --collection chat-session-docs --scope temporary --session-id 1
school-ai ingest ./docs/handbooks --collection openwebui-docs --scope temporary --external-session-id owui-chat-1
```

`--scope temporary` では `--session-id` または `--external-session-id` のどちらかが必要です。
