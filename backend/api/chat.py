...
def create_chat_session(request: ChatSessionCreateRequest, db: Session = Depends(get_db)):
    return ChatService(db, create_llm_adapter()).create_session(
        title=request.title,
...
def list_chat_sessions(db: Session = Depends(get_db)):
    return ChatService(db, create_llm_adapter()).list_sessions()
...
def get_chat_session(session_id: int, db: Session = Depends(get_db)):
    service = ChatService(db, create_llm_adapter())
    try:
...
def delete_chat_session(session_id: int, db: Session = Depends(get_db)):
    try:
        ChatService(db, create_llm_adapter()).delete_session(session_id=session_id)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail="チャットセッションが見つかりません。") from exc
    return None
...
def list_chat_messages(session_id: int, db: Session = Depends(get_db)):
    try:
        return ChatService(db, create_llm_adapter()).list_messages(session_id=session_id)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail="チャットセッションが見つかりません。") from exc
...
def send_chat_message(
    ...
    try:
        session, user_message, assistant_message, tool_suggestions = ChatService(
            db, create_llm_adapter()
        ).send_message(
...
def send_chat_message_direct(
    ...
    return send_chat_message(session_id=request.session_id, request=request, db=db)


@router.get("/chat/tools", response_model=list[ChatToolInfoOut])
def list_chat_tools(db: Session = Depends(get_db)):
    return _tool_infos_to_schema(ChatService(db, create_llm_adapter()).list_available_tools())
